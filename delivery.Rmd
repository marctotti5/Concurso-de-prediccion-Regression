...
This is a reproducible RMarkdown of the building of the final mode.
This script assumes there exists a `train.csv` file and a `test.csv` next to it containting the data.
The output is the requested `predicted_prices.xlsx` file.

Note that this is a compressed version of longer `R` script with plots and more tools for observing the data.
The intended goal of this RMarkdown is to be able to reproduce the model and predict the test data consistently.

```{r}
rm(list = ls())
set.seed(12345)

# Pacman is a package to manage packages, if they are installed they are loaded,
# and if not, they are installed first and then loaded. Install it first before
# running the code!
#
...
# GAM predicts on the scale of the (log price), so we exp() it
