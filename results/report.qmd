...
@fig-median-price-by-level shows the relationship between prices and the category levels by the use of the median for each level (less affected by outliers).
...
We focus now on missing values on the remaining variables.
@fig-missing-values shows those remaining with any of them.
...
Percentage of missing values across the remaining variables.
:::

`LotFrontage` is the variable with the highest number of missing values, likely due to the fact that measuring this value was not always possible.
`Electrical`, `MasVnrArea` and `MasVnrType` show a small percentage of missing values.
In particular `MasVnrArea` and `MasVnrType` have the exact same number of missing values, which raises the question: are they always misssing at the same time?

::: {#fig-distribution-of-missing-values layout-ncol=2}
```{r}
...
Indeed, @fig-distribution-of-missing-values shows that the two variables are missing for the same indices.
In the case of `LotFrontage`, there might be a hidden pattern difficult to see at first, glance, thus @fig-missing-grouped.

::: {#fig-missing-grouped layout-ncol=2}
```{r}
...
Missing values grouped by neighborhood and by lot shape.
No clear pattern can be established from them.
:::

Since there is still no apparent behavior, the final question before removing it is if it affects the price sales.
First, @fig-lots-compared shows that the variable `LotArea` is not affected by the missing values in `LotFrontage`.
...
Distribution of the lot area grouped by the lot frontage availability.
Distributions are similar, so there is no bias in missingness.
:::

What about prices?
@fig-prices-by-missingness shows that there is a quantifyiable difference between the two groups, best represented by their means (`r round((172400/159500 - 1) * 100)`% higher in the case with missing values).

::: {#fig-prices-by-missingness}
```{r}
...
Distribution of prices grouped by values missing or not.
Observations with missing values have a higher median, which cannot be ignored.
:::
